// ignore: unused_import
import 'package:intl/intl.dart' as intl;
import 'app_localizations.dart';

// ignore_for_file: type=lint

/// The translations for Arabic (`ar`).
class AppLocalizationsAr extends AppLocalizations {
  AppLocalizationsAr([String locale = 'ar']) : super(locale);

  @override
  String get home => 'الرئيسية';

  @override
  String get projects => 'المشاريع';

  @override
  String get blogs => 'المدونات';

  @override
  String get aboutMe => 'معلومات عني';

  @override
  String get contactme => ' تواصل معي';

  @override
  String get walid => ' وليد ';

  @override
  String get mobileAppDeveloper => 'مطور تطبيقات هاتف';

  @override
  String get mobileAppDeveloperDesc =>
      'أنا مطور تطبيقات جوال لدي خبرة لمدة عام في تطوير تطبيقات فلاتر ودارت عالية الجودة. أستمتع بإنشاء تطبيقات مبتكرة وسهلة الاستخدام تحل مشاكل حقيقية للمستخدمين';

  @override
  String get hammoutiWalid => 'حموتي وليد';

  @override
  String get techStack => 'المهارات التقنية';

  @override
  String get techStackDesc =>
      'فيما يلي بعض التقنيات والأدوات التي لدي خبرة بها:';

  @override
  String get experince => 'الخبرة والمهارات';

  @override
  String get experincedesc =>
      'نظرة على خبرتي عبر مجالات تطوير مختلفة - من الهاتف إلى الذكاء الاصطناعي.';

  @override
  String get poweredByFlutter => 'مدعوم من Flutter';

  @override
  String get aboutmeTitle => 'حموتي وليد، مطور تطبيقات وML';

  @override
  String get aboutmeDesc =>
      'مرحبًا، أنا حموتي وليد، مطور تطبيقات شغوف ومتحمس للذكاء الاصطناعي من الجزائر. أحب بناء تطبيقات ذكية وسهلة الاستخدام تجمع بين واجهات Flutter الجميلة وقوة التعلم الآلي. أنا أستكشف حاليًا كيف يمكن للذكاء الاصطناعي تحسين التطبيقات في العالم الحقيقي - من الأنظمة الذكية إلى أدوات الأتمتة. هدفي هو إنشاء منتجات رقمية تجعل الحياة أسهل وأكثر ذكاءً وترابطًا.';

  @override
  String get reachMe => 'تواصل معي';

  @override
  String get welcometext => 'مرحبًا بكم في محفظتي! ';

  @override
  String get herotitle => 'السلام عليكم، أنا';

  @override
  String get hammouti => 'حموتي';

  @override
  String get mobileDevelopment => 'تطوير تطبيقات الهاتف';

  @override
  String get mobileDevText1 =>
      'بناء تطبيقات هاتف متعددة المنصات باستخدام Flutter و Dart';

  @override
  String get mobileDevText2 =>
      'دمج Supabase للمصادقة وقاعدة البيانات في الوقت الفعلي';

  @override
  String get mobileDevText3 =>
      'تصميم سمات فاتحة وداكنة متجاوبة لتحسين تجربة المستخدم';

  @override
  String get mobileDevText4 =>
      'استخدمت Cubit و BLoC و Provider و Riverpod لإدارة الحالة القابلة للتطوير';

  @override
  String get aiMachineLearning => 'الذكاء الاصطناعي وتعلم الآلة';

  @override
  String get aiText1 => 'أتعلم حاليًا وأستكشف تقنيات تعلم الآلة';

  @override
  String get aiText2 =>
      'إجراء تحليل البيانات والمعالجة المسبقة باستخدام pandas و NumPy';

  @override
  String get aiText3 =>
      'تصور مجموعات البيانات وأداء النماذج باستخدام Matplotlib';

  @override
  String get desktopDevelopment => 'تطوير تطبيقات سطح المكتب';

  @override
  String get desktopText1 => 'بناء نظام إدارة الطلاب باستخدام C و GTK3';

  @override
  String get desktopText2 =>
      'تطوير آلة حاسبة للجبر الخطي باستخدام Python و Tkinter';

  @override
  String get copyright => '© 2025 حموتي وليد. جميع الحقوق محفوظة.';

  @override
  String get reach => 'تواصل';

  @override
  String get me => ' معي ';

  @override
  String get contactDescription =>
      'لا تتردد في التواصل! يمكنك الاتصال بي عبر البريد الإلكتروني أو الهاتف أو من خلال قنوات التواصل الاجتماعي الخاصة بي. يسعدني أن أسمع منك.';

  @override
  String get email => 'w.walidcontacts@gmail.com';

  @override
  String get phone => '67 57 26 62 5 213+';

  @override
  String get contactLets => 'دعونا';

  @override
  String get contactTalk => ' نتحدث ';

  @override
  String get contactSubtitle =>
      'هل لديك شيء لتقوله؟ اترك رسالة مجهولة ولنحتفظ بالأمر بيننا 👀';

  @override
  String get contactYourName => 'اسمك';

  @override
  String get contactNameValidation => 'الرجاء إدخال اسمك';

  @override
  String get contactYourMessage => 'رسالتك';

  @override
  String get contactMessageValidation => 'الرجاء إدخال رسالتك';

  @override
  String get contactMessageHint => 'اكتب رسالتك هنا...';

  @override
  String get contactSendButton => 'إرسال الرسالة';

  @override
  String get contactSuccess => 'تم إرسال الرسالة بنجاح!';

  @override
  String get contactError =>
      'فشل في إرسال البريد الإلكتروني. الرجاء المحاولة مرة أخرى.';

  @override
  String get whatIBuilt => 'ما الذي أنشأته';

  @override
  String get projectsSubtitle =>
      'نظرة على رحلتي — بناء مشاريع هادفة تحل المشكلات وتلهم الأفكار.';

  @override
  String get projectRiguelniTitle => 'Riguelni - منصة العمل الحر';

  @override
  String get projectRiguelniDesc =>
      'ريغيلني هي منصة حديثة للعمل الحر تربط بين المحترفين المهرة والعملاء الباحثين عن خدمات. توفر بيئة آمنة وسهلة الاستخدام حيث يمكن للمستخدمين شراء وبيع الخدمات، مما يجعلها حلاً مرنًا للمستقلين والشركات على حد سواء. مع ميزات قوية مثل الدردشة الفورية ولوحة التحكم البسيطة، تهدف ريغيلني إلى تسهيل المعاملات وإدارة المشاريع.';

  @override
  String get projectFluentNowTitle => 'FluentNow — رفيقك الذكي لتعلم اللغات';

  @override
  String get projectFluentNowDesc =>
      'FluentNow هو تطبيق حديث صُمم للمدارس الخاصة لتعليم اللغات لربطها بطلابها. يسمح للمستخدمين باستكشاف الدورات المتاحة، واكتشاف البرامج الجديدة، والبقاء على اطلاع بالعروض والأحداث والأخبار. يمكن للطلاب التسجيل مباشرة عبر التطبيق والوصول إلى جميع المعلومات بسهولة وأناقة.';

  @override
  String get projectPortfolioTitle => 'ملفي الشخصي على الويب باستخدام Flutter';

  @override
  String get projectPortfolioDesc =>
      'ملف شخصي حديث وديناميكي يعرض مشاريعي ومهاراتي ورحلتي الإبداعية في تطوير البرمجيات. من واجهات Flutter البديهية إلى الحلول الويب المتجاوبة، يعكس هذا الملف شغفي ببناء تقنيات ذكية تركز على المستخدم. استكشف أعمالي وتعرف على خبرتي وكيف أحول الأفكار إلى منتجات رقمية مؤثرة.';

  @override
  String get projectEvotingTitle => 'خلفية تشفيرية لنظام تصويت إلكتروني آمن';

  @override
  String get projectEvotingDesc =>
      'بناء الخلفية التشفيرية لنظام تصويت إلكتروني آمن في إطار مشروع هندسي بالسنة الثالثة بمدرسة ENSTA الجزائر. شمل العمل تطبيق تشفير RSA بالمفتاح العام، والتوقيعات العمياء لضمان سرية بطاقات الاقتراع، ونظام مصادقة الناخبين عبر دوال التجزئة. تعاونت مع مطور واجهة أمامية لدمج واجهة برمجة التطبيقات التشفيرية مع تطبيق الويب.';

  @override
  String get viewResume => 'عرض السيرة الذاتية';
}
